...
public class PhaseID {
	...
	/**
	 * This first crude phase identification is used to get a decent starting 
	 * location.  Only trusted phases are used and they are generally 
	 * associated with the theoretical arrival of the same name.
	 */
	private void pFirst() {
	...
	private void reID() {
		...
		System.out.println("\n\tClusters:");
		tTime = ttList.get(0);
		winMin = tTime.tt-tTime.window;
		winMax = tTime.tt+tTime.window;
		ttBeg = 0;
		ttLen = 1;
		pickBeg = -1;
		pickLen = 0;
		...
		for(int j=1; j<ttList.size(); j++) {
			...
			if(tTime.tt-tTime.window <= winMax) {
				winMin = Math.min(winMin, tTime.tt-tTime.window);
				winMax = Math.max(winMax, tTime.tt+tTime.window);
				ttLen++;
			} else {
	...
	/**
	 * Generate combinations of picks or theoretical arrivals to compare with all 
	 * possible combinations of theoretical arrivals or picks.  For example, if 
	 * there are 3 picks and 5 theoretical arrivals, the theoretical arrivals will 
	 * be taken 3 at a time until all possible combinations in the original order 
	 * have been generated.  Each combination will be treated as a trial phase 
	 * identification of the picks to those theoretical arrivals.
	 * 
	...
	private void permut(int pickBeg, int pickLen, int ttBeg, int ttLen) {
		...
		for(int j=0, i=pickBeg; j<pickLen; j++, i++) pickGrp[j] = 
				group.picks.get(i);
		TTimeData[] ttGrp = new TTimeData[ttLen];
		for(int j=0, i=ttBeg; j<ttLen; j++, i++) ttGrp[j] = ttList.get(i);
  ...
  private void kPermutOfN(TTimeData[] ttGrp, int len, int beg, TTimeData[] ttPermut, 
	...
	/**
	 * Create all k-permutations of n objects, where k is the length of pickPermut 
	 * and n is the length of pickGrp.  Note that this algorithm is recursive.  The 
	 * variables len and beg are primarily for internal use.  For the caller, len 
	 * should be the length of the result (pickPermut) and beg should be 0.  This 
	 * algorithm has been taken from StackOverflow.  It was posted by user935714 
	 * on 20 April 2016.
	 * 
  ...
  private void kPermutOfN(Pick[] pickGrp, int len, int beg, Pick[] pickPermut, 
	...
	private void setFoM(Pick[] pickGrp, TTimeData[] ttGrp) {
		...
		for(int j=0; j<ttGrp.length; j++) {
			prob = LocUtil.ttResModel(pickGrp[j].tt-ttGrp[j].tt, 0d, ttGrp[j].spread);
	...
	private double idAmplitude(Pick pick, TTimeData tTime) {
		...
		if(pick != lastPick) {
			...
			if(primary) System.out.print(" Pri");
			if(generic) System.out.print(" Gen");
		} else {
		...
		if(!tTime.dis) amp = tTime.observ;
			...
			amp = LocUtil.DOWNWEIGHT*tTime.observ;
		...
		if((!pick.idCode.equals(tTime.phCode) || generic) && 
			...
			if(generic && primary) {
				 ...
				 */
				if(phGroup.equals(tTime.phGroup) || phGroup.equals(tTime.auxGroup) || 
...
/**
 * Helper class keeping track of the limits and contents of one cluster 
 * of theoretical and observed phases.  Clusters are defined by groups 
 * of theoretical phases that are isolated from each other in time.  
 * Observed phases are then associated with clusters.
 * 
 * @author Ray Buland
 *
 */
