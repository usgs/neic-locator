...
/**
 * The Stepper class manages the rank-sum-estimator logic needed to refine the 
 * hypocenter.
 * 
 * @author Ray Buland
 *
...
public class Stepper {
  /**
   * An Event object containing the event to use when performing Stepper 
   * calculations.
   */
  private Event event;
   ...
   * performing Stepper calculations.
   */  
  private Hypocenter hypo;  

  /**
   * An AuxLocRef object containing auxiliary locator information used when 
   * performing Stepper calculations.
   */ 
  private AuxLocRef auxLoc;
  
  /**
   * A Cratons object holding the geographic boundaries of continental 
    * cratons.  
   */ 
  ...
  /**
   * The Stepper constructor. Set the event, phaseID logic, and auxiliary 
   * locator information to the provided values
   * 
   * @param event An Event object containing the event to use when performing 
   *               Stepper calculations.
   * @param phaseIDLogic A PhaseID object containing the phase identification 
   *                      logic.
   * @param auxLoc An AuxLocRef object containing auxiliary locator information 
   *                used when performing Stepper calculations.
   */
  ...
  /**
   * The Stepper phase identification function. Sets the tectonic flag and Bayesian 
   * depth parameters.  Calculate the median residual (origin time correction), 
   * rank-sum-estimator dispersion, and rank-sum-estimator direction of steepest 
   * descents.  This initial version also sets the reference dispersion value in 
   * the hypocenter.
   * 
   * @param otherWeight A double value holding the weight for phases that don't 
   *                     match the current phase identification or the current 
   *                     phase group (higher weights make changing to an "other" 
   *                     phase easier)
   * @param stickyWeight A double value holding the weight for an exact match 
   *                      (higher weights make changing the current 
   *                      identification harder.
   * @param reidentifyPhases A boolean flag indicating whether to perform a full 
   *                          phase re-identification, if false try not to change 
   *                          identifications
   * @param updateResWeights A boolean flag indicating whether to the residual 
   *                          weights
   * @return A LocStatus object containing the status value.
   * @throws Exception On an illegal source depth
   */
  public LocStatus doPhaseIdentification(double otherWeight, double stickyWeight, 
      boolean reidentifyPhases, boolean updateResWeights) throws Exception {
    LocStatus status = internalPhaseID(otherWeight, stickyWeight, reidentifyPhases, 
        updateResWeights);
  ...
  /**
   * The Stepper internal phase identification function. Sets the tectonic flag 
   * and Bayesian depth parameters.  Then calculates the median residual (origin 
   * time correction), rank-sum-estimator dispersion, and rank-sum-estimator 
   * direction of steepest descents.  For calls from makeStep, we don't want to 
   * update the reference dispersion in the hypocenter just yet.
   * 
   * @param otherWeight A double value holding the weight for phases that don't 
   *                     match the current phase identification or the current 
   *                     phase group (higher weights make changing to an "other" 
   ...
   * @throws Exception On an illegal source depth
   */
  private LocStatus internalPhaseID(double otherWeight, double stickyWeight, 
      boolean reidentifyPhases, boolean updateResWeights) throws Exception {
    ...
    // Reidentify phases.
    event.setHasPhaseIdChanged(phaseIDLogic.phaseID(otherWeight, stickyWeight, 
        reidentifyPhases, updateResWeights));
    ...
    if (LocUtil.useDecorrelation) {
      ...
      hypo.setStepDirectionUnitVector(
          projectedRankSumEstimator.compSteepestDescDir(
          hypo.getDegreesOfFreedom()));
    } else {
      // Demedian the raw residuals.
      residualsMedian = rawRankSumEstimator.computeMedian();
      ...
      // Get the steepest descent direction.
      hypo.setStepDirectionUnitVector(rawRankSumEstimator.compSteepestDescDir(
          hypo.getDegreesOfFreedom()));
    }
  ...
  /**
   * This function makes a step from the current hypocenter to the optimal 
   * rank-sum-estimator dispersion minimum based on linearized residual estimates.
   * 
  ...
  public LocStatus makeStep(int stage, int iteration) throws Exception {
    ...
    hypo.setStepLength(Math.max(hypo.getStepLength(), 2d 
        * LocUtil.CONVERGENCESTAGELIMITS[stage]));
    rSumEstResult = linearStep.stepLength(hypo.getStepDirectionUnitVector(), 
        hypo.getStepLength(), LocUtil.CONVERGENCESTAGELIMITS[stage], 
        LocUtil.STEPLENSTAGELIMITS[stage], hypo.getEstimatorDispersionValue());
    
    // This weird special case appears once in a while.
    if (rSumEstResult.getDispersion() >= hypo.getEstimatorDispersionValue() 
        && rSumEstResult.getStepLength() 
        < LocUtil.CONVERGENCESTAGELIMITS[stage]) {
    ...
    event.updateHypo(rSumEstResult.getStepLength(), 
        rSumEstResult.getMedianResidual());

    // Reidentify phases and get the non-linear rank-sum-estimator parameters 
    // for the new hypocenter.
    if (internalPhaseID(0.01d, 5d, false, false) 
        == LocStatus.INSUFFICIENT_DATA) {
      return LocStatus.INSUFFICIENT_DATA;
    }
    ...
    do {
      ...
      if (damp * hypo.getStepLength() <= LocUtil.CONVERGENCESTAGELIMITS[stage] 
        ...
        // Set the exit status.
        if (rSumEstResult.getDispersion() <= LocUtil.ALMOSTCONVERGED 
            * hypo.getEstimatorDispersionValue() && hypo.getStepLength() 
            <= LocUtil.CONVERGENCESTAGELIMITS[stage]) {
      ...
      // Do the damping.
      hypo.setNumOfTimesStepLengthDampening(
          hypo.getNumOfTimesStepLengthDampening() + 1);
      hypo.resetHypo(lastHypoAudit);
    ...
    } while (rSumEstResult.getDispersion() 
  ...
  /**
   * This function sets the location environment by determining if the location 
   * is in a craton or tectonic area and setting the Bayesian depth.
   */
  protected void setLocEnvironment() {
    ...
    if (LocUtil.deBugLevel > 0) {
      System.out.format("\tBayes: %5.1f %5.3f %b\n", hypo.getBayesianDepth(), 
          hypo.getBayesianDepthWeight(), event.getIsDepthManual());
    }
  }
  
  /**
   * This function logs the current step.  Note that this logging is the 
   * principle means of debugging Locator problems.  In the long run, this 
   * output should be entered into the error log and only printed for a debug 
  ...
  private void logStep(String id, int stage, int iteration, LocStatus status) {
    ...
    if (used >= hypo.getDegreesOfFreedom()) {
      hypo.setEstimatorRMSEquivalent(hypo.getEstimatorDispersionValue() 
    ...
    if (LocUtil.deBugLevel > 0) {
      System.out.format("\n%s: %1d %2d %5d %8.4f %8.4f %6.2f del= %5.1f %6.1f " 
          + "rms= %6.2f %s\n", id, stage, iteration, used, hypo.getLatitude(), 
          hypo.getLongitude(), hypo.getDepth(), hypo.getHorizontalStepLength(), 
          hypo.getVerticalStepLength(), hypo.getEstimatorRMSEquivalent(), 
          status);
    }
  }
}
