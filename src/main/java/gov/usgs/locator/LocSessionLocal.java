...
/**
 * Manage the external Locator files.  This includes changing the slab model 
...
public class LocSessionLocal {
  String[] modelFileNames = {"slabmaster.txt", "slabtilted.txt"};
	 ...
	 * @param slabRes String specifying the slab model resolution in samples 
	 * per degree
	...
	/**
	 * Get the desired slab model resolution.  This generally requires a serialization 
	 * from a file.  If the serialization files doesn't exist, the raw input file is 
	 * read and serialized out for next time.
	 * 
	 * @param slabRes String specifying the slab model resolution in samples 
	 * per degree
	 * @return A slabs object containing all the slab model data for the requested 
	 * resolution
	 * @throws IOException If the serialized I/O fails in any way
	 * @throws ClassNotFoundException If the serialized slab master object doesn't 
	 * exist
  ...
  /**
   * Read in the master slab model.  Note that each area is done in a 
   * rectangular grid, which is sparse (the points outside the slab are 
   * flagged by NaNs).  The algorithm simple accumulates rows, squeezing 
   * the NaNs out of each row, while looking for the start of the next 
   * area.
   */
...
//	LOGGER.fine(area);
  ...
  /**
   * Read the tilted slab file and append a summary to the end of the 
   * master slab areas.  Note that the tilted slabs file is huge and very 
   * dense.  Instead of sorting it out, we read the whole thing in, 
   * create a latitude-longitude grid, sort the samples into the grid 
   * elements, and extract the handful of numbers we need.
   */
