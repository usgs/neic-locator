...
public class RankSumEstimator {
  /**
   * An int containing the last index of the weightedResiduals array, cached for use in
   * dispersion().
   */
  private int lastIndex;

  /**
   * An int containing the length of the weightedResiduals ArrayList, cached for use in
   * deMedianDesignMatrix().
   */
  private int weightedResidualsLength;

  /**
   * An int containing the half index of the weightedResiduals array, cached for use in
   * deMedianDesignMatrix().
   */
  ...
  public double computeMedian() {
    ...
    if (weightedResiduals.size() < 2) {
      ...
      if (weightedResiduals.size() == 0) {
        return 0d;
      } else if (weightedResiduals.size() == 1 && weightedResiduals.get(0).getIsBayesianDepth()) {
        return 0d;
      }
    }
  ...
  /**
   * This function computes the normalized median absolute deviation or spread of the travel-time
   * residuals (excluding the Bayesian depth residual). Note that the median is remembered from the
   * median method.
   *
   * @return A double containing the spread of travel-time residuals stored in weightedResiduals.
   */
  ...
  /**
   * This function removes the median from the residuals. Note that the median is remembered from
   * the median method.
   */
  ...
  /**
   * This function removes the medians from each row of the design matrix. Note that the positions
   * of the median values corresponds to the positions for the residual, not their design values.
   * Obviously, the median method must be invoked before deMedianDesignMatrix to determine these
   * positions and no other method that re-sorts the residuals (i.e., spread and penalty) may be
   * invoked in between.
   */
  public void deMedianDesignMatrix() {
    ...
    if (weightedResidualsLength % 2 == 0) {
      for (int i = 0; i < medianValues.length; i++) {
        medianValues[i] =
            0.5d
                * (weightedResiduals.get(halfIndex - 1).getSpatialDerivatives()[i]
  ...
  /**
   * This function computes the rank-sum estimator penalty function or dispersion. Note that the
   * median is remembered from the median method.
   *
   * @return A double containing the the rank-sum estimator dispersion / penalty function
   */
  ...
  /**
   * This function computes the dispersion of the current set of weighted residuals. The dispersion
   * is a convenient summary of the current data fit.
   *
   * @return A double containing the the dispersion / penalty function of the current set of
   *     weighted residuals
   */
  ...
  /**
   * This function computes the steepest descents direction. This depends on the weightedResiduals
   * sort order created the computeDispersionValue method, so median and spread methods should not
   * be invoked between.
   *
   * @param degreesOfFreedom Number of degrees of freedom (degreesOfFreedom = 2 for fixed depth,
   *     degreesOfFreedom = 3 for free depth)
   * @return A double[] containing the the steepest descents direction unit vector
   */
  public double[] compSteepestDescDir(int degreesOfFreedom) {
    ...
    for (int j = 0; j < weightedResiduals.size(); j++) {
      ...
      for (int i = 0; i < degreesOfFreedom; i++) {
        stepUnitVector[i] +=
            scores[j] * weightedResidual.getWeight() * weightedResidual.getDeMedSpaDerivatives()[i];
      }
    }
  ...
  /**
   * This function computes the median of the linear estimates of the travel-time residuals
   * (excluding the Bayesian depth residual).
   *
  ...
  public double computeLinEstMedian() {
    ...
    if (weightedResidualsLength % 2 == 0) {
      ...
      linearEstimatesMedian = weightedResiduals.get(halfIndex).getLinEstResidual();
      return linearEstimatesMedian;
    }
  }
  ...
  /**
   * This funtion interpolates the score generating function onto the data available.
   *
  ...
  private void makeScores(int numData) {
    ...
    // problems.
    final double[] scoreGenP = {
      0d, 0.1375d, 0.1625d, 0.1875d, 0.2125d, 0.2375d, 0.2625d, 0.2875d, 0.3125d, 0.3375d, 0.3625d,
      0.3875d, 0.4125d, 0.4375d, 0.4625d, 0.4875d, 0.5125d, 0.5375d, 0.5625d, 0.5875d, 0.6125d,
      0.6375d, 0.6625d, 0.6875d, 0.7125d, 0.7375d, 0.7625d, 0.7875d, 1d
    };
    final double[] scoreGenF = {
      0.0775d, 0.0775d, 0.1546d, 0.5328d, 0.8679d, 1.1714d, 1.4542d, 1.7266d, 1.9987d, 2.2802d,
      2.5803d, 2.9068d, 3.2657d, 3.6603d, 4.0912d, 4.5554d, 5.0470d, 5.5572d, 6.0754d, 6.5906d,
      7.0919d, 7.5702d, 8.0194d, 8.4365d, 8.8223d, 9.1812d, 9.5207d, 9.5974d, 9.5974d
    };
    ...
    for (int j = 0; j < numData; j++) {
      ...
      scores[j] =
          (p - scoreGenP[k - 1])
                  * (scoreGenF[k] - scoreGenF[k - 1])
                  / (scoreGenP[k] - scoreGenP[k - 1])
              + scoreGenF[k - 1];
      average += scores[j];
    }
