...
public enum LocStatus {
  ...
  /**
   * External (exit) status for a successful location that was essentially the  
   * same as the starting location.
   */
  DID_NOT_MOVE(2),          // Normal completion, but didn't change location

  /**
   * External (exit) status representing that there were internal errors related 
   * to computing error bars or error ellipsoids.
   */
  ERRORS_NOT_COMPUTED(3),  // Normal completion, but error computation failed

  /**
   * External (exit) status catch all.  If this is returned, it probably means 
   * that a new internal status wasn't converted to an external status.
   */
  UNKNOWN_STATUS(4),        // Just in case...

  /**
   * External (exit) status meaning that the location failed due to lack of data  
   * or some other problem.
   */
  LOCATION_FAILED(101),    // Location failed (singular or insufficient data)

  /**
   * External (exit)  status meaning that the event input data was broken in  
   * some indecipherable way.
   */
  BAD_EVENT_INPUT(110),    // Bad read (or other input) on the event data

  /**
   * External (exit)  status meaning that the travel-time data wasn't where it  
   * should have been.
   */
