...
public class Slabs implements Serializable {
  ...
  /**
   * For analyst input, the spread is interpreted as a 99th percentile. Here the raw numbers are one
   * sigma (68%). To make the slab statistics compatible, they must be multiplied by 3.
   */
  double bayesSpread;
  ...
  /**
   * Get a list of slab depths for a point. Note that with overturned slabs, etc. it is possible to
   * sample at least two slabs for one geographic point.
   *
  ...
  /**
   * Deciding which Bayesian depth to use is complicated. There are four possibilities: 1) there is
   * no slab (use the shallow depth), 2) the slab is close to the surface (use a combination of the
   * slab and shallow depth), 3) the trial depth is deep, so always use the slab depth, and 4) use
   * the shallow or slab depth depending on which is closest to the trial depth.
   *
  ...
  public double getBayesDepth(double latitude, double longitude, double depth) {
    ...
    if (depths == null) {
      ...
      if (slabDepth.getEqDepth() <= LocUtil.SLABMERGEDEPTH) {
         ...
         */
        double deepest =
            slabDepth.getEqDepth() + 3d * (slabDepth.getUpper() - slabDepth.getEqDepth());
        ...
        bayesSpread =
            3d
                * Math.max(
                    slabDepth.getEqDepth() - slabDepth.getLower(),
