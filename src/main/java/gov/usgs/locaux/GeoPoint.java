...
/**
 * Holds one geographical point for the nearest ZoneStats grid point (or cell center).
 *
 * @author Ray Buland
 */
public class GeoPoint implements Comparable<GeoPoint> {
  ...
  /**
   * Construct a new geographical point by colatitude and longitude and compute the Earth flattened
  ...
  /**
   * Using the Earth flattening, compute the Cartesian distance in degrees between this GeoPoint and
   * a trial GeoPoint.
   *
   * @param trial Trial geographical point
   * @return Distance between this geographic point and the trial geographic point in degrees
   */
  public double getDistance(GeoPoint trial) {
    return Math.sqrt(Math.pow(trial.getX() - x, 2d) + Math.pow(trial.getY() - y, 2d));
  }

  /**
   * Using the Earth flattening, compute the Cartesian distance in degrees between this GeoPoint and
   * the Earth flattening origin (which is always the reference point).
   *
   * @return Distance between this geographical point and the Earth flattening origin in degrees
   */
   ...
   *
   * @param index Select the depth parameter to include (0 = mean depth, 1 = lower, 2 = upper, and 3
   *     = spread)
   * @return A 3-vector containing colatitude, longitude, and maximum ZoneStats earthquake depth
   */
  ...
  public String toString() {
    if (bayesDepth == null) {
      return String.format("%5.2f %6.2f => (%5.2f, %5.2f) dist = %4.2f", lat, lon, x, y, dist);
    } else {
      return String.format(
          "%5.2f %6.2f => (%5.2f, %5.2f) dist = %4.2f" + " depth = %6.2f +/- %6.2f",
          lat, lon, x, y, dist, bayesDepth.getDepth(), bayesDepth.getSpread());
    }
  }
}
